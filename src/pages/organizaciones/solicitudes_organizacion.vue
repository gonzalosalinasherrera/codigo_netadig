<template>
  <div>
    <Breadcrumbs main="Usuarios" title="Lista de Solicitudes"/>
...
export default{
    ...
    cancelSol(to_delete){
        ...
        axios.patch('http://127.0.0.1:8000/api/solicitud_organizacion/' + this.orgid[3] + '/',{
          user_to_join: this.members
        })
        .then(resp => {
          this.$swal({
				    title:"Solcitud Eliminada",
			    })
            console.log(resp)
            this.$router.push('/organizaciones/perfil/' + this.orgid[3] + '/')
          })
    ...
    getSolicitudes(){
      ...
      .then((resp) => {
        ...
        this.sol = this.solicitudes.user_to_join
        this.miembros();
      })
      .catch((error) => {
        console.log(error)
  ...
  computed:{
    filteredList(){
      return this.profile.filter(u =>{
        return u.usuario.first_name.toLowerCase().includes(this.search.toLowerCase()) ||
        u.usuario.last_name.toLowerCase().includes(this.search.toLowerCase()) ||
        u.usuario.email.toLowerCase().includes(this.search.toLowerCase())
      })
    }
  },
}
